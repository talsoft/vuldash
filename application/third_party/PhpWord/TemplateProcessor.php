...
class TemplateProcessor
{
    ...
    public function setImg( $strKey, $img) {
        ...
        {
            $toAddImg.='<w:br/><w:t>'.$this->cleanString($img['dataImg']).'</w:t><w:br/>';
        }
    ...
    public function cloneTable($search, $numberOfClones) {
        ...
        $tableStart = $this->findTableStart($tagPos, true);
    ...
    private function findTableStart($offset, $forward = false)
    {
        ...
        if($forward) {
            $tableStart = strpos($this->tempDocumentMainPart, "<w:tbl ", $offset);
            if (!$tableStart) {
                $tableStart = strpos($this->tempDocumentMainPart, "<w:tbl>", $offset);
            }
        } else {
            $tableStart = strrpos($this->tempDocumentMainPart, "<w:tbl ", ((strlen($this->tempDocumentMainPart) - $offset) * -1));
            if (!$tableStart) {
                $tableStart = strrpos($this->tempDocumentMainPart, "<w:tbl>", ((strlen($this->tempDocumentMainPart) - $offset) * -1));
            }
            if (!$tableStart) {
    ...
    {   
        $tableEnd = strpos($this->tempDocumentMainPart, "</w:tbl>", $offset) + 8;
        return $tableEnd;
    }  
    
    public function setChart($chartName, $values)
    {        
        $chart = $this->fixBrokenMacros($this->zipClass->getFromName('word/charts/' . $chartName . '.xml'));

        for($x = 0; $x < sizeof($values); $x++)
            for($y = 0; $y < sizeof($values[$x]); $y++)                
                $chart = preg_replace('/(<c:v>)(99)(<\/c:v>)/', '${1}'.$values[$x][$y].'$3', $chart, 1);
            
        $this->zipClass->addFromString('word/charts/' . $chartName . '.xml', $chart);
    }
}
